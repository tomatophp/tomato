<x-tomato-admin-layout>
    <x-slot name="header">
        {{trans('tomato-admin::global.crud.edit')}} {{trans('tomato-roles::global.roles.single')}} #{{$model['id']}}
    </x-slot>
    ...
    <x-splade-form class="flex flex-col space-y-4 mb-4" action="{{route('admin.roles.update', $model['id'])}}" method="post" :default="$model">
        ...
        <x-splade-submit label="{{trans('tomato-admin::global.crud.update')}} {{trans('tomato-roles::global.roles.single')}}" :spinner="true" />
    </x-splade-form>
</x-tomato-admin-layout>
